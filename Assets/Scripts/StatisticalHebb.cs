...
public class StatisticalHebb : Act
{
    ...
    void Start()
    {
        ...
        outSynapse = new RenderTexture(synapseHebb.width, synapseHebb.height, 0, RenderTextureFormat.ARGB32);
        outSynapse.enableRandomWrite = true;
        outSynapse.Create();
        outSynapse.filterMode = FilterMode.Point;
    ...
    private void InitHebb(int n)
    {
        shader.SetTexture(kiHebb, "outSynapseHebb", outSynapse);
        shader.SetBuffer(kiHebb, "XY", XY);
        shader.SetInt("number", n);
    }
    ...
    private void CalculateHabb()
    {
        shader.Dispatch(kiHebb, 1, 1, 1);
        Graphics.Blit(outSynapse, synapseHebb);
    }
